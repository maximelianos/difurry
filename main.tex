...
\small{Семинарист \bfend Елена Александровна Павельева\\
...
($\lambda = a \pm ib$) & $y_k(x) = x^{k - 1}e^{ax}\cos bx$,\\
...
\circled{\romannum{1}}\quad $\mat{Y}' = A\mat{Y}$\\\vspace{0.5em}
}
...
Нельзя оставлять $\mathbb{C}$ собств. значения! Делать так:\\
$\lambda_i = \alpha\pm i\beta \Rightarrow \mat{Y}_i = \mat{\alpha}_i \cdot (\cos\beta t + i\sin\beta t)e^{\alpha t} = \mat{\alpha}_1 + i\mat{\alpha}_2 \Rightarrow$\\
$\mat{Y}_{\text{одн}} = c_1\mat{\alpha}_1 + c_2\mat{\alpha}_2\quad \forall c_1, c_2$\\
Итого $\mat{Y}_{\text{одн}} = c_1 \mat{\alpha}_1 e^{\lambda_1} + \ldots + c_n \mat{\alpha}_n e^{\lambda_n}\quad \forall c_1,\ldots c_n$

{\centering \circled{\romannum{2}}\quad $\mat{Y}' = A\mat{Y} + \mat{F}$\\\vspace{0.5em}} % Матрицы неоднор
...
$\mat{Y_1} = \mat{Q}_{m+s}(t) e^{\newgamma t}$\\ ($s=$ кратность $\newgamma$)
}\vspace{0.5em}

\circled{2}\,
\parbox[t][]{\htext}{Второй спец. вид. $\mat{F} = (\mat{P_m}(t) \cos \beta t + \mat{Q_l}(t) \sin \beta t) \cdot e^{\alpha t}\\ \Rightarrow$
$\mat{Y_1} = (\mat{R}_{k+s}(t) \cos \beta t + \mat{T}_{k+S}(t)\sin\beta t)\cdot e^{\alpha t}\\ k = max(m, l)$\\ ($s=$ кратность $\newgamma$)
}\vspace{0.5em}

\circled{3}\,
\parbox[t][]{\htext}{$\mat{F}$ \bfend сумма специальных видов $\textinfers \mat{Y_1} = \sum y_{\text{спец}}$ % Комментарии нужны??
}\vspace{0.5em}
